<template>
    <div class="o-customerManagementAgreementAddressFormHu max-w-contain mx-auto mt-20 py-20 px-10 md:px-40 text-gray-150 text-sm bg-white border-gray-50 border-1">
...
<script>
    ...
    export default {
        name: 'OCustomerManagementAgreementAddressFormHu',
        ...
        validations() {
            return {
                address1Data: {
                    ...
                    maxLength: maxLength(formHelper.maxChar.hu.address_1)
                },
                address2Data: {
                    required: requiredIf(() => this.labels?.Address2?.Required),
                    maxLength: maxLength(formHelper.maxChar.hu.address_2)
                },
                cityData: {
                    required: requiredIf(() => this.labels?.City?.Required),
                    maxLength: maxLength(formHelper.maxChar.hu.city)
                },
                deliveryInstructionsData: {
                    required: requiredIf(() => this.labels?.DeliveryInstructions?.Required),
                    maxLength: maxLength(formHelper.maxChar.hu.deliveryInstructions)
                },
                postalCodeData: {
                    required: requiredIf(() => this.labels?.PostalCode?.Required),
                    postalCode: (val) => {
                        if(!val) return true;
                        return formHelper.isValidPostalCode(val, 'hu');
                    }
                }
            };
        },
