...
<script>
    ...
    const experianAddressFields = [ 'address_1Data', 'suburbData', 'stateData', 'postalCodeData' ];

    export default {
        name: 'MImeaAddressFormAu',
        ...
        validations() {
            return {
                address_1Data: {
                    ...
                    maxLength: maxLength(formHelper.maxChar.au.address_1)
                },
                address_2Data: {
                    required: requiredIf(() => this.labels?.Address_2?.Required),
                    maxLength: maxLength(formHelper.maxChar.au.address_2)
                },
                postalCodeData: {
                    required: requiredIf(() => this.labels?.PostalCode?.Required),
                    postalCode: (val) => {
                        if(!val) return true;
                        return formHelper.isValidPostalCode(val, 'au');
                    }
                },
                stateData: {
                ...
                suburbData: {
                    ...
                    maxLength: maxLength(formHelper.maxChar.au.suburb)
                }
            };
        },
        data() {
        ...
        computed: {
            ...
            addressEnteredFormatted() {
                ...
                const suburb = this.addressEntered.suburb ? this.addressEntered.suburb + lineBreak : '';
                const state = this.addressEntered.state ? this.addressEntered.state + ' ' : '';
                const postalCode = this.addressEntered.zipCode || '';

                return address1 + address2 + suburb + state + postalCode;
            },
            statePlaceholder() {
        ...
        methods: {
            checkAddressAgainstExperianSuggestionList() {
                const userDataFormatted = `${this.address1Data} ${this.suburbData}  ${this.stateData} ${this.postalCodeData}`;
            ...
            populateFields(response) {
                ...
                this.address_1Data = addressObject.addressLine1;
                this.address_2Data = addressObject.addressLine2;
                this.suburbData = addressObject.locality;
                this.stateData = addressObject.province;
