...
<script>
    ...
    const experianAddressFields = [ 'address_1Data', 'suburbData', 'cityData', 'postalCodeData' ];

    export default {
        name: 'MImeaAddressFormNz',
        ...
        validations() {
            return {
                address_1Data: {
                    ...
                    maxLength: maxLength(formHelper.maxChar.nz.address_1)
                },
                address_2Data: {
                    required: requiredIf(() => this.labels?.Address_2?.Required),
                    maxLength: maxLength(formHelper.maxChar.nz.address_2)
                },
                cityData: {
                ...
                postalCodeData: {
                    ...
                    postalCode: (val) => {
                        if(!val) return true;
                        return formHelper.isValidPostalCode(val, 'nz');
                    }
                },
                suburbData: {
                    required: requiredIf(() => this.labels?.Suburb?.Required),
                    maxLength: maxLength(formHelper.maxChar.nz.suburb)
                }
            };
        },
        data() {
        ...
        computed: {
            ...
            addressEnteredFormatted() {
                ...
                return address1 + address2 + suburb + city + postalCode;
            }
        },
        watch: {
            addressEntered() {
        ...
        methods: {
            checkAddressAgainstExperianSuggestionList() {
                const userDataFormatted = `${this.address1Data} ${this.suburbData}${this.suburbData ? ' ' : ''}${this.cityData} ${this.postalCodeData}`;
            ...
            populateFields(response) {
                ...
                this.cityData = addressObject.province;
