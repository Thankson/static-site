    ...
    <div>
        <o-access
            ...
            <template v-slot:heading>
                {{ data.CreateAccountHeader }}
            </template>

            <template v-slot:progress-step-1>
                <span v-html="data.NavigationStep1"></span>
            </template>

            <template v-slot:step-1>
                <m-access-identify :is-cn="isCn" :loading="loading" :gtm-id="184" :create-account="true">
                    <template v-slot:heading>
                    ...
                    <template v-slot:customer-id-error>
                        <span v-html="data.Step1ErrorInvalidCustomerID"></span>
                    </template>
                    ...
                    <template v-if="data.Step1NotAMemberLink" v-slot:info-link>
                        <span v-html="data.Step1NotAMemberLink"></span>
                    </template>

                    <template v-if="data.Step1NotAMemberHelp" v-slot:info-link-tooltip>
                        <span v-html="data.Step1NotAMemberHelp"></span>
                    </template>
                </m-access-identify>
            </template>

            <template v-slot:progress-step-2>
                <span v-html="data.NavigationStep2"></span>
            </template>

            <template v-slot:step-2>
                <m-access-verify :is-cn="isCn" :loading="loading" :gtm-id="184" :create-account="true">
                    <template v-slot:heading>
                    ...
                    <template v-if="data.Step2NotAMemberLink" v-slot:info-link>
                        <span v-html="data.Step2NotAMemberLink"></span>
                    </template>

                    <template v-if="data.Step2NotAMemberHelp" v-slot:info-link-tooltip>
                        <span v-html="data.Step2NotAMemberHelp"></span>
                    </template>
            ...
            <template v-slot:progress-step-3>
                <span v-html="data.NavigationStep3"></span>
            </template>
            ...
            <template v-slot:progress-final>
                <span v-html="data.NavigationStep4"></span>
            </template>
        </o-access>
...
<script>
    ...
    // API handlers
    import localizationApi from '@api/routes/customeraccount/createaccountlocalization';
    import passwordApi from '@api/routes/customeraccount/createaccount';
    ...
    export default {
        ...
        methods: {
            ...
            async localization() {
                const response = await localizationApi.get();
            ...
            async password(params) {
                this.loading = true;
                const response = await passwordApi.post(params);
